/* Tơ Hồng — Onboarding v4 · 3-step wizard
   Step 1 · Thông tin cặp đôi
   Step 2 · Kế hoạch đám cưới
   Step 3 · Khám phá phong cách (3a: grid, 3b: result) */
        ...
        {opts.map(o => {
          ...
          return (
            <button key={o.val} onClick={() => onPick(o.val)} style={{
              ...
              <Icon name={o.icon} size={15} color={on ? '#fff' : 'var(--ink-500)'} sw={2} />
              {o.label}
            </button>
          );
        })}
...
function OBStep3Result({ profile, data, onRestart, onFinish }) {
  ...
  return (
    <>
      ...
      <div style={{ flexShrink: 0, borderTop: '1px solid var(--line-100)', padding: '12px 20px 20px', display: 'flex', flexDirection: 'column', gap: 8 }}>
...
export function Onboarding({ open, onDone }) {
  ...
  useEffect(() => {
    if (open) { setStep(1); setData({ ...OB_DEFAULTS }); setLikes(new Set()); }
  }, [open]);
